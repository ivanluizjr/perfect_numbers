...
void main() {
  ...
  test('should return Right([6]) for range 1..10', () {
    when(() => repository.findInRange(1, 10)).thenReturn(Right([six]));
    final result = useCase(1, 10);
  ...
  test('should return Right([6, 28, 496]) for range 1..1000', () {
    when(
      () => repository.findInRange(1, 1000),
    ).thenReturn(Right([six, twentyEight, fourNinetySix]));
    final result = useCase(1, 1000);
    expect(result.isRight, isTrue);
    expect(result.rightOrNull?.length, 3);
    expect(
      result.rightOrNull?.map((r) => r.number).toList(),
      [6, 28, 496],
    );
    verify(() => repository.findInRange(1, 1000)).called(1);
  });

  test('should return Right([]) for range with no perfect numbers', () {
    when(() => repository.findInRange(29, 100)).thenReturn(const Right([]));
    final result = useCase(29, 100);
    expect(result.isRight, isTrue);
    expect(result.rightOrNull, isEmpty);
    verify(() => repository.findInRange(29, 100)).called(1);
  });

  test('should return Left(ValidationFailure) when start > end', () {
    final result = useCase(100, 10);
    expect(result.isLeft, isTrue);
    expect(result.leftOrNull, isA<ValidationFailure>());
    verifyNever(() => repository.findInRange(any(), any()));
  });

  test('should return Left(ValidationFailure) when start < 1', () {
    final result = useCase(0, 100);
    expect(result.isLeft, isTrue);
    expect(result.leftOrNull, isA<ValidationFailure>());
    verifyNever(() => repository.findInRange(any(), any()));
  });

  test('should return Left(ValidationFailure) when start is negative', () {
    final result = useCase(-10, 100);
    expect(result.isLeft, isTrue);
    expect(result.leftOrNull, isA<ValidationFailure>());
    verifyNever(() => repository.findInRange(any(), any()));
  });

  test('should return Right for equal start and end (single number)', () {
    when(() => repository.findInRange(6, 6)).thenReturn(Right([six]));
    final result = useCase(6, 6);
    expect(result.isRight, isTrue);
    expect(result.rightOrNull?.length, 1);
    expect(result.rightOrNull?.first.isPerfect, isTrue);
  });

  test('should propagate Left from repository', () {
